...
const renderAIAdvisor = (preRound, postRound, pricedConversion, state, strictlyPreFounderPct) => {
    ...
    aiLoadingTimeout = setTimeout(() => {
        ...
        if (totalFounderPctPost < 0.5) {
            const warningMessage = totalFounderPctPre >= 0.5 
                ? "Founders have dropped below 50% majority ownership in this round." 
                : "Founders are below 50% majority ownership.";
            insights.push(`
                <div class="insight-item" style="color: #0d0a40; margin: 0 0 1.25rem 0; font-family: 'Inter', sans-serif;">
                    <div class="insight-danger" style="color: #dc2626; font-weight: 500; display: flex; align-items: center; gap: 0.5rem; margin-bottom: 20px;">${warningMessage}</div>
                </div>
            `);
        }
...
const prepareReportData = () => {
    ...
    // Synchronize SAFE shares between Pre and Post if a priced round exists.
    // This ensures consistency between the UI and the PDF.
    preRound.safes = preRound.safes.map(preSafe => {
        const postSafe = pricedTable.safes.find(ps => ps.id === preSafe.id);
        return postSafe ? { ...preSafe, shares: postSafe.shares } : preSafe;
    });

    // Recalculate pre-round totals for ownership calculation consistency
    preRound.total.shares = preRound.common.reduce((a, c) => a + (c.shares || 0), 0) + 
                            preRound.safes.reduce((a, s) => a + (s.shares || 0), 0);
...
window.downloadPDF = async function() {
    try {
        ...
        console.log("Fetching from EquityList API...");
        ...
        if (result.success) {
            ...
            throw new Error(result.message || "API failed to generate PDF");
        }
    } catch (error) {
        ...
        if (modalMode === 'download') {
            ...
            const result = await safeFetch('/api/v1/safe_calculator/send_email', {
                ...
                body: JSON.stringify({ 
                    ...
                    summaryData: { 
                        firstName: firstName || 'there',
                        founderOwnership: report_data.summary.ownershipPost,
                        founderDilution: report_data.summary.dilution,
                        postMoney: report_data.summary.postMoney,
                        totalRaised: report_data.summary.totalRaised
                    } 
                })
            });
